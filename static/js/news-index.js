...
// // apis 
// const API_KEY = "sm3ysXY_IzEkb_FvnyolLVxtAjIItIxCgSdnN4bkJ4c";
...
(function () {
  ...
  $(document).ready(function () {
    ...
    container.find(".desc img[src*='feedsportal']").each(function () {
      $(this).remove();
    });
    ...
    container.find(".box").each(function () {
      var q = $(this).find(".title").text() + " " + $(this).find(".desc").text();
      $(this).attr("data-search", q.toLowerCase());
    });
  
    // click+remember
    container.find(".title a").click(function () {
      var id = $(this).data("id");
      if (localStorage) {
        localStorage.last_id = id;
      }
      container.find("#item-" + id).addClass("marked");
    });
  
    // share
    container.find(".share a").attr("target", "_blank");
  
    // live search
    window.timer = null;
    $("#q").bind("change keyup paste", function () {
      ...
      window.timer = setTimeout(function () {
        if (q.length >= 3) {
          container.find(".box:hidden").addClass("item").show();
          var items = container
            .find(".box:not([data-search*='" + q + "'])")
            .hide()
            .removeClass("item");
          ...
          if (container.find(".box:visible").length == 0) {
            $("#notfound").show();
          } else {
            $("#notfound").hide();
          }
    ...
    // worcloud
    $("#wordcloud a.word").click(function () {
      $("#q").val($(this).data("word")).select().change();
      return false;
    });
  
    // tick update check
    window.setInterval(function () {
      $.get(ROOT + "tick.php", function (r) {
        // updated
        if (r != HASH) {
          HASH = r;
  ...
  $(window).ready(function () {
    window.setTimeout(function () {
      ...
      container.find(".desc img").each(function () {
        $(this).addClass("ready").attr("src", $(this).data("src"));
      });
      ...
      imagesLoaded(container.get(0), function () {
        ...
        for (var n = 0; n < imgs.length; n++) {
          if (parseFloat($(imgs[n]).attr("width")) < 50) {
            $(imgs[n]).remove();
          }
        }
  
        var msnry = new Masonry($("#news").get(0), {
          itemSelector: ".item",
          gutter: 50,
        });
        msnry.on("layoutComplete", function () {
          // rememberance
          if (localStorage && "last_id" in localStorage) {
            var item = container
              .find("#item-" + localStorage.last_id)
              .addClass("marked");
